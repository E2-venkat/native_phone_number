package com.venkat.phonenumberandsms;
...
/**
 * PhoneNumberAndSmsPlugin
 */
public class PhoneNumberAndSmsPlugin implements FlutterPlugin, ActivityAware, MethodCallHandler {

    private static final int PHONE_HINT_REQUEST = 11012;
    private static final String channelName = "phone_number_sms";
    ...
    private final PluginRegistry.ActivityResultListener activityResultListener = new PluginRegistry.ActivityResultListener() {
        ...
        public boolean onActivityResult(int requestCode, int resultCode, Intent data) {
            Log.d("onActivityResult", "we are now looking for the  on activity working or not" );
            if (requestCode == PhoneNumberAndSmsPlugin.PHONE_HINT_REQUEST && pendingHintResult != null) {
                if (resultCode == Activity.RESULT_OK && data != null) {
                    Credential credential = data.getParcelableExtra(Credential.EXTRA_KEY);
                    final String phoneNumber = credential.getId();
                    Log.d("phoneNumber", "we are now looking for the  " + phoneNumber);
    ...
    public PhoneNumberAndSmsPlugin() {
    }

    private PhoneNumberAndSmsPlugin(Registrar registrar) {
    ...
    public static void registerWith(Registrar registrar) {
        new PhoneNumberAndSmsPlugin(registrar);
    }
    ...
    public void onMethodCall(MethodCall call, @NonNull final Result result) {
        switch (call.method) {
            case "requestPhoneHint":
                Log.d("requestPhoneHint", "we are now inside request phone hint class ");
                Log.d("pendingHintResult", "we are now looking for the  " + result);
            ...
            case "listenForCode":
                ...
                task.addOnSuccessListener(new OnSuccessListener<Void>() {
                    ...
                    public void onSuccess(Void aVoid) {
                        ...
                        broadcastReceiver = new SmsBroadcastReceiver(new WeakReference<>(PhoneNumberAndSmsPlugin.this),
                                smsCodeRegexPattern);
            ...
            case "getAppSignature":
                com.venkat.phonenumberandsms.AppSignatureHelper signatureHelper = new com.venkat.phonenumberandsms.AppSignatureHelper(activity.getApplicationContext());
    ...
    private void requestHint() {
        ...
        Log.d("requestHint", "we are now checking " + isSimSupport());
        ...
        try {
            activity.startIntentSenderForResult(intent.getIntentSender(),
                    PhoneNumberAndSmsPlugin.PHONE_HINT_REQUEST, null, 0, 0, 0);

            Log.d("startIntentSenderForResult", "we are now checking " + intent.getIntentSender());
    ...
    public boolean isSimSupport() {
        ...
        Log.d("checking the getLineNumber", String.valueOf(telephonyManager.getSimState()));
        return !(telephonyManager.getSimState() == TelephonyManager.SIM_STATE_ABSENT);
    }

    private void setupChannel(BinaryMessenger messenger) {
        channel = new MethodChannel(messenger, PhoneNumberAndSmsPlugin.channelName);
    ...
    private static class SmsBroadcastReceiver extends BroadcastReceiver {
        ...
        final WeakReference<PhoneNumberAndSmsPlugin> plugin;
        final String smsCodeRegexPattern;

        private SmsBroadcastReceiver(WeakReference<PhoneNumberAndSmsPlugin> plugin, String smsCodeRegexPattern) {
            this.plugin = plugin;
